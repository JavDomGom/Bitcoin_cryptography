...
\section{Almacenamiento en ficheros}
    ...
    En este ejemplo se ha empleado un sistema operativo 100\% libre\footnote{https://www.gnu.org/distros/free-distros.es.html} GNU/Linux. Si se listan los archivos que contiene el directorio $\sim$/.bitcoin/blocks/ aparecerá una lista como la siguiente:
...
000000e0  ...}
    ...
    Estos son los primeros bytes del primer archivo de la cadena de bloques de Bitcon, los primeros datos de llamado \textit{bloque génesis}. En este archivo no solo está el primer bloque de la cadena, hay muchos más, ordenados cronológicamente según se van minando y añadiendo a la cadena.
    
    Siguiendo con el ejemplo del bloque \texttt{\#}286819, en condiciones normales, es decir, siempre y cuando a la cadena de bloques descargada no se le haya aplicado algún tipo de \textit{prune}\footnote{https://en.bitcoin.it/wiki/Running\_Bitcoin\#Command-line\_arguments} o podado, sus datos se encuentran en el archivo \textbf{blk00116.dat}. Los datos del bloque que se quiere analizar no tienen por qué aparecer necesariamente en el inicio del archivo, es bastante probable que los datos comiencen muchos bytes más adelante o en un \textit{offset} mucho mayor desde el inicio del archivo. En este caso concretamente, los datos comienzan a partir del byte número 93725126, en el \textit{offset} número \texttt{059621c6}. En el siguiente ejemplo se ejecuta el programa \textbf{hexdump} con la opción \textbf{-s} seguido del número de bytes desde el que se quiere empezar a visualizar datos, y la opción \textbf{-n} para indicar el número total de bytes que se quieren mostrar por pantalla, en este caso solo 288 bytes, la opción \textbf{-C} es para obtener una salida standard.
    \begin{figure}[H]
    ...
    En la columna izquierda se ve el \textit{offset} correspondiente a cada línea, en la parte central los datos del bloque en formato hexadecimal y la columna de la derecha cada uno de los 16 bytes de datos representados en caracteres ASCII\footnote{https://www.ieee.li/computer/ascii.htm}. Se ha resaltado en diferentes colores los diferentes segmentos de datos. En color rojo aparecen los 4 bytes que representan el dato \textbf{magicID}, a continuación en color verde los 4 siguientes bytes el dato \textbf{size} que representa el tamaño final de este bloque. El segmento que sigue en color turquesa son los 80 bytes que representan el \textbf{header}. En color azul un único byte que representa el número de transacciones registradas en el bloque y finalmente en color morado se han resaltado todos los datos correspondientes a las transacciones del bloque. Los datos no terminan en el \textit{offset} \texttt{059622e6}, se ha truncado el bloque en ese punto para une mejor visualización en este documento, pues la cantidad de datos del bloque mostrada en este formato podría abarcar demasiado espacio.
\end{document}
