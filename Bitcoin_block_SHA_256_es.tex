...
\section{Estructura de datos de un bloque}
    ...
    Cuando se habla sobre un bloque de la cadena de bloques de Bitcoin en realidad se está hablando sobre una serie de datos ordenados en 5 bloques principales. Los 5 bloques son los siguientes:
    
    \begin{figure}[H]
    \centering
        $\underbrace{\texttt{[magicID]}}_{4\ bytes}\underbrace{\texttt{[headerLenght]}}_{4\ bytes}\underbrace{\texttt{[Cabecera]}}_{80\ bytes}\underbrace{\texttt{[Número\ de\ tx]}}_{Variable}\underbrace{\texttt{[Tx\ data]}}_{Resto\ de\ bytes}$
    \end{figure}
